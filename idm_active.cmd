...
::
::   Homepages: https://github.com/codehubcloud/idm_active.git
::
::
::
::============================================================================
...
start %SystemRoot%\Sysnative\cmd.exe /c ""!cmdf!" %* r1"
exit /b
)
...
setlocal EnableDelayedExpansion
start %SystemRoot%\SysArm32\cmd.exe /c ""!cmdf!" %* r2"
exit /b
)
...
if defined args (
for %%A in (%args%) do (
if /i "%%A"=="-el"  set elev=1
if /i "%%A"=="/res" set reset=1
if /i "%%A"=="/frz" set freeze=1
if /i "%%A"=="/act" set activate=1
)
)

for %%A in (%activate% %freeze% %reset%) do (if "%%A"=="1" set unattended=1)
...
set ncs=1
if %winbuild% LSS 10586 set ncs=0
if %winbuild% GEQ 10586 reg query "HKCU\Console" /v ForceV2 %nul2% | find /i "0x0" %nul1% && (set ncs=0)
...
set    "Blue="Blue" "white""
set  "White="Black" "Gray""
set  "Green="Black" "Green""
set "Yellow="Black" "Yellow""
)

set "nceline=echo: &echo ==== ERROR ==== &echo:"
set "eline=echo: &call :color1 %Red% "==== ERROR ====" &echo:"
...
echo "!batf!" | find /i "!temp_dir!" %nul1% && (
if /i not "!work!"=="!temp_dir!" (
%eline%
...
%psc% "$f=[io.file]::ReadAllText('!batp!') -split ':PowerShellTest:\s*';iex ($f[1])" | find /i "FullLanguage" %nul1% || (
%eline%
...
%nul1% fltmc || (
if not defined elev %psc% "start cmd.exe -arg '/c \"!ps_arg!\"' -verb runas" && exit /b
%eline%
...
for %%# in (%args%) do (if /i "%%#"=="-qedit" set quedit=1)
...
%launchcmd% "%d1% %d2% %d3% %d4% & cmd.exe '/c' '!ps_arg! -qedit'" &exit /b
:skipQE
...
echo:
call :color1 %Green% "Enter a menu option in the Keyboard [1,0] :"
choice /C:10 /N
if !errorlevel!==2 rem
...
if %activate%==1 (set frz=0&goto :activate)
if %freeze%==1 (set frz=1&goto :activate)
...
echo:            ___________________________________________________
echo:         
call :color2 %White% "             " %Green% "Enter a menu option in the Keyboard [1,2,3,4,5,0]"
choice /C:123450 /N
set err_level=%errorlevel%

if %err_level%==6 exit /b
if %err_level%==5 start https://github.com/WindowsAddict/IDM-Activation-Script & start https://massgrave.dev/idm-activation-script & goto MainMenu
if %err_level%==4 start https://www.internetdownloadmanager.com/download.html & goto MainMenu
if %err_level%==3 goto reset
if %err_level%==2 (set frz=0&goto :activate)
if %err_level%==1 (set frz=1&goto :activate)
goto :MainMenu
...
set time=
for /f %%a in ('%psc% "(Get-Date).ToString('yyyyMMdd-HHmmssfff')"') do set time=%%a

echo:
echo Creating backup of CLSID registry keys in %SystemRoot%\Temp

reg export %CLSID% "%SystemRoot%\Temp\_Backup_HKCU_CLSID_%time%.reg"
if not %HKCUsync%==1 reg export %CLSID2% "%SystemRoot%\Temp\_Backup_HKU-%_sid%_CLSID_%time%.reg"

call :delete_queue
%psc% "$sid = '%_sid%'; $HKCUsync = %HKCUsync%; $lockKey = $null; $deleteKey = 1; $f=[io.file]::ReadAllText('!batp!') -split ':regscan\:.*';iex ($f[1])"
...
echo %line%
echo:
call :color1 %Green% "The IDM reset process has been completed."
...
if %frz%==0 if %unattended%==0 (
echo:
echo %line%
echo:
echo      Activation is not working for some users and IDM may show fake serial nag screen.
echo:
call :color2 %White% "     " %Green% "Its recommended to use Freeze Trial option instead."
echo %line%
echo:
...
echo:
if not exist "%IDMan%" (
call :color1 %Red% "IDM [Internet Download Manager] is not Installed."
...
call :color1 %Red% "Unable to connect internetdownloadmanager.com, aborting..."
goto done
)
call :color1 %Gray% "Ping command failed for internetdownloadmanager.com"
echo:
)
...
%psc% "$sid = '%_sid%'; $HKCUsync = %HKCUsync%; $lockKey = 1; $deleteKey = $null; $toggle = 1; $f=[io.file]::ReadAllText('!batp!') -split ':regscan\:.*';iex ($f[1])"

if %frz%==0 call :register_IDM

call :download_files
if not defined file_exist (
%eline%
...
%psc% "$sid = '%_sid%'; $HKCUsync = %HKCUsync%; $lockKey = 1; $deleteKey = $null; $f=[io.file]::ReadAllText('!batp!') -split ':regscan\:.*';iex ($f[1])"

echo:
echo %line%
echo:
if %frz%==0 (
call :color1 %Green% "The IDM Activation process has been completed."
echo:
call :color1 %Gray% "If the fake serial screen appears, use the Freeze Trial option instead."
) else (
call :color1 %Green% "The IDM 30 days trial period is successfully freezed for Lifetime."
echo:
call :color1 %Gray% "If IDM is showing a popup to register, reinstall IDM."
)
...
echo:
echo:
if %unattended%==1 timeout /t 2 & exit /b

if defined terminal (
call :color1 %Yellow% "Press 0 key to return..."
choice /c 0 /n
) else (
call :color1 %Yellow% "Press any key to return..."
pause %nul1%
)
goto MainMenu
...
timeout /t 1 %nul1%
set /a attempt+=1
if exist "%file%" set file_exist=1&exit /b
